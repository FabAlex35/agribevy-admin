...
export async function middleware(req) {
    ...
    const accessToken = cookies.adminAccToken || null;
    const refreshToken = cookies.adminRefToken || null;
    ...
    console.log(decodedAccess,decodedRefresh,!decodedAccess,!decodedRefresh);
    ...
    if(!decodedAccess){
        if(!decodedRefresh){
            ...
            if (req.method != "GET" && url.pathname != "/api/auth/refresh") {
                ...
                return NextResponse.json({ message: "Refresh" }, { status: 401 });
            }
            else if(url.pathname == "/"){
                return NextResponse.redirect(new URL("/portal/admindashboard", req.url))
            }
        }
    }

    if(decodedAccess && decodedAccess.exp > currentTime || decodedRefresh && decodedRefresh.exp > currentTime){
        return url.pathname == ("/") ? NextResponse.redirect(new URL("/portal/admindashboard", req.url)) : response;
    }
    return response;
